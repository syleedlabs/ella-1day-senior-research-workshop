...
hero:
  ...
  tagline: "오늘 워크숍이 끝나면, 시니어 시장을 국내·해외로 대신 조사해주는 리서치 에이전트가 손에 있을 거예요!"
  actions:
    - theme: brand
      text: 시작하기
      link: /prologue
...
### 소요시간
약 **3시간** (프롤로그 5분 + Part 1 기초·리서치 에이전트 실행 + Part 2 개념·실무)

### 사전 준비물
- 노트북, 인터넷 연결
- 클로드 코드 설치 (Pro/Max 구독) — 이미 설치돼 있어도 OK, 무엇을 시킬지가 핵심이에요
- **조사해보고 싶은 시니어 주제 2~3개** — "시니어 주거", "시니어 식품", "시니어 여행"처럼 러프해도 괜찮아요. 떠오르지 않으시면 에이전트가 샘플을 제안해드립니다.

::: tip 준비물이 가벼운 이유
리서치 에이전트는 **웹에서 직접 조사**합니다. 나리님이 자료를 모아 오실 필요가 없어요. 주제만 던져주시면 됩니다.
:::

### 학습 모드
- 프롬프트가 완벽하지 않아도 괜찮습니다. 에이전트는 **클로드 코드와의 대화로 언제든 수정**할 수 있어요.
- 모르는 부분 있으면 멈추고 물어보세요.
- 에이전트가 만든 브리프·리포트는 `output/` 폴더에 쌓입니다. 워크숍이 끝나도 그대로 남아요.
